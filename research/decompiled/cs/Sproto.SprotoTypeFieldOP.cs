...
public class SprotoTypeFieldOP
{
	...
	{
		int num = max_field_count / slot_bits_size;
		if (max_field_count % slot_bits_size > 0)
		{
			num++;
		}
		has_bits = new uint[num];
	}

	private int _get_array_idx(int bit_idx)
	{
		int num = has_bits.Length;
		return bit_idx / slot_bits_size;
	}
	...
	{
		int num = _get_array_idx(field_idx);
		int num2 = _get_slotbit_idx(field_idx);
		uint num3 = has_bits[num];
	...
	public void set_field(int field_idx, bool is_has)
	{
		...
		{
			uint num4 = (uint)(1 << num2);
			has_bits[num] = num3 | num4;
		}
		else
		{
			uint num5 = (uint)(~(1 << num2));
			has_bits[num] = num3 & num5;
		}
	}
