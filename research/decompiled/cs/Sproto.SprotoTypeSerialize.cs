...
public class SprotoTypeSerialize
{
	...
	{
		data[header_idx - 2] = (byte)(fn & 0xFF);
		data[header_idx - 1] = (byte)((fn >> 8) & 0xFF);
	}

	private void write_header_record(int record)
	{
		data[header_idx + header_cap - 2] = (byte)(record & 0xFF);
		data[header_idx + header_cap - 1] = (byte)((record >> 8) & 0xFF);
		header_cap += 2;
		index++;
	}
