using Sproto;

namespace SprotoType;

public class ret_title_req_level_up
{
	public class request : SprotoTypeBase
	{
		private static int max_field_count = 2;

		private long _title_level;

		private long _title_exp;

		public long title_level
		{
			get
			{
				return _title_level;
			}
			set
			{
				has_field.set_field(0, is_has: true);
				_title_level = value;
			}
		}

		public bool HasTitle_level => has_field.has_field(0);

		public long title_exp
		{
			get
			{
				return _title_exp;
			}
			set
			{
				has_field.set_field(1, is_has: true);
				_title_exp = value;
			}
		}

		public bool HasTitle_exp => has_field.has_field(1);

		public request()
			: base(max_field_count)
		{
		}

		public request(byte[] buffer)
			: base(max_field_count, buffer)
		{
			decode();
		}

		protected override void decode()
		{
			int num = -1;
			while ((num = deserialize.read_tag()) != -1)
			{
				switch (num)
				{
				case 0:
					title_level = deserialize.read_integer();
					break;
				case 1:
					title_exp = deserialize.read_integer();
					break;
				default:
					deserialize.read_unknow_data();
					break;
				}
			}
		}

		public override int encode(SprotoStream stream)
		{
			serialize.open(stream);
			if (has_field.has_field(0))
			{
				serialize.write_integer(title_level, 0);
			}
			if (has_field.has_field(1))
			{
				serialize.write_integer(title_exp, 1);
			}
			return serialize.close();
		}
	}
}
