...
public class MenuSceneController : SingletonUnity<MenuSceneController>
{
	...
	public void ChooseRecommendServer()
	{
		...
		if (nearTimezoneServer.Count > 0)
		{
			...
			game_server = ((list == null || list.Count <= 0) ? GetRecommendServer(nearTimezoneServer) : GetRecommendServer(list));
			if (game_server != null)
			{
				PlayerData.CurGameServerData = new game_server();
				PlayerData.CurGameServerData.serverId = game_server.serverId;
				PlayerData.CurGameServerData.serverIP = game_server.serverIP;
				PlayerData.CurGameServerData.serverPort = game_server.serverPort;
				PlayerData.CurGameServerData.serverName = game_server.serverName;
				PlayerData.CurGameServerData.serverState = game_server.serverState;
				if (game_server.HasNewServer)
				{
					PlayerData.CurGameServerData.newServer = game_server.newServer;
				}
			}
			else
			{
	...
	public game_server GetRecommendServer(List<game_server> uselist)
	{
		...
		if (num2 == 0)
		{
			...
			game_server = list[index];
		}
		else
		{
			int num3 = UnityEngine.Random.Range(0, num2);
			game_server = list[list.Count - 1];
